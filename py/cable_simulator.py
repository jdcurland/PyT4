...
l2 = conf.L2socket(iface=network_adapter)
l3 = conf.L3socket(iface=network_adapter)
...
# sniff(iface=network_adapter, prn=t4interface, store=0)
# myfilter="tcp[tcpflags] & (tcp-syn|tcp-ack|tcp-push) == tcp-syn"

# sniff(iface=network_adapter, prn=t4interface, store=0)
testbook_traffic = AsyncSniffer(iface=network_adapter, prn=t4interface, store=0)

testbook_traffic.Start()
# time.sleep(1)
# bytepackage = [0] #echo
#bytepackage = [0,21,0,6,0]

# testpacket(bytepackage)
# sniff(iface="VirtualBox Host-Only Network", filter='arp', stop_filter= lambda x: stopfilter(x))